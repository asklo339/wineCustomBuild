...
echo "Selected Wine branch: $WINE_BRANCH"
echo "Selected Proton branch: $PROTON_BRANCH"
...
if [ "$TERMUX_GLIBC" = "true" ]; then
    ...
    echo "Applying address space patch"
    patch -d wine -Np1 < "${scriptdir}/termux-p10.patch" && \
    echo "Applying path change patch"
    patch -d wine -Np1 < "${scriptdir}/patch-avcodec.patch" && \
    echo "Applying avcodec patch"
    ## Proton is based on Wine 9.0 stable release so some of the updates are not required.
    patch -d wine -Np1 < "${scriptdir}/pathfix-p10.patch" || {
        echo "Error: Failed to apply one or more patches."
        exit 1
    }
else
...
# Add debug output to see what's being built
echo "Building Wine, please wait..."
${BWRAP64} make -j$(nproc)
echo "Installing Wine..."
${BWRAP64} make install

# Debug the installation
echo "Directory structure of the built Wine:"
find "${BUILD_DIR}"/wine-"${BUILD_NAME}"-amd64 -type d -maxdepth 2 | sort
...
cd "${BUILD_DIR}"/build32-tools || exit 1
